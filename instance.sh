...
aws ec2 describe-instances --filters "Name=tag:Name,Values=$INSTANCE_NAME" | jq .Reservations[].Instance[].State.Name | grep running &>/dev/null
if [ $? -eq 0 ];
    then 
...
aws ec2 describe-instances --filters "Name=tag:Name,Values=$INSTANCE_NAME" | jq .Reservations[].Instance[].State.Name | grep stopped &>/dev/null
if [ $? -eq 0 ];
    then 
        echo -e "\e[1;33mInstance : $instances already exists\e[0m"
        exit 1
